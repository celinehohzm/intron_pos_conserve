...
for protein_id in $(cat "$orthologs_protein_id_list"); do
    ...
    python3 1_extract_table.py whole_table.txt ${protein_id} > extracted_table.txt

    # Process multi-line fasta files, outputs org_aa_sequence.fasta
    python3 1.5_extract_seq_from_fasta.py amino_acid.fa > org_aa_sequence.fa
    ...
    python3 2.1_extract_n_insert_introns.py extracted_table.txt org_aa_sequence.fa ${first_protein_id}
